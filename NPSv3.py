...
# Adicionando a coluna "Taxa de Respostas" com distribuição 70% SIM e 30% NÃO
df['Taxa de Respostas'] = np.random.choice(['SIM', 'NÃO'], size=len(df), p=[0.7, 0.3])

# Definindo Data_Respostas como nulo para respostas NÃO
df.loc[df['Taxa de Respostas'] == 'NÃO', 'Data_Respostas'] = np.nan

# Definindo Data_Respostas e Nota como nulos para respostas NÃO
df.loc[df['Taxa de Respostas'] == 'NÃO', ['Data_Respostas', 'Nota']] = np.nan

# Adicionando uma coluna "Satisfeito" com base na nota
df['Satisfeito'] = df['Nota'].apply(lambda x: 'Sim' if x >= 7 else 'Não')
